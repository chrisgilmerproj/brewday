...
def get_yeast_pitch_calculation(
    ...
    data = model.get_yeast_pitch_rate(
        original_gravity=og,
        final_volume=fv,
    ...
    data = model.get_starter_volume(
        available_cells=data[u"cells"], starter_volume=sv, original_gravity=sg
    )
    ...
    resulting_pitch_rate = model.get_resulting_pitch_rate(
        starter_cell_count=end_cell_count, original_gravity=sg, final_volume=fv
    )
    data.update(model.types)
    msg = textwrap.dedent(
        u"""\
...
def get_parser():
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.036,
        help=u"Starter Original Gravity (default: %(default)s)",
    )  # noqa
    parser.add_argument(
        u"--sv",
        metavar=u"V",
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help=u"Number of days since yeast manufacture (default: %(default)s)",
    )  # noqa
    parser.add_argument(
        u"--model",
        metavar=u"M",
...
def main(parser_fn=get_parser, parser_kwargs=None):
    ...
    if args.units not in [IMPERIAL_UNITS, SI_UNITS]:
        print(u"Units must be in either {} or {}".format(IMPERIAL_UNITS, SI_UNITS))
        sys.exit(1)
    ...
    if args.model == u"white":
        ...
        print(
            u"Unknown Yeast Growth Model '{}', must be 'white' or 'kaiser'".format(
                args.model
            )
        )  # noqa
        sys.exit(1)
