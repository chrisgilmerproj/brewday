...
class TestHops(unittest.TestCase):
    ...
    def test_eq(self):
        hop1 = Hop(u"cascade", percent_alpha_acids=0.07)
        hop2 = Hop(u"cascade", percent_alpha_acids=0.07)
        self.assertEquals(hop1, hop2)

    def test_ne_name(self):
        hop1 = Hop(u"cascade", percent_alpha_acids=0.07)
        hop2 = Hop(u"centennial", percent_alpha_acids=0.07)
        self.assertTrue(hop1 != hop2)

    def test_ne_percent_alpha_acids(self):
        hop1 = Hop(u"cascade", percent_alpha_acids=0.07)
        hop2 = Hop(u"cascade", percent_alpha_acids=0.14)
        self.assertTrue(hop1 != hop2)
    ...
    def test_to_dict(self):
        ...
        expected = {u"name": u"centennial", u"percent_alpha_acids": 0.14}
    ...
    def test_hop_no_percent_alpha_acids(self):
        with self.assertRaises(HopException) as ctx:
            Hop(u"centennial")
        self.assertEquals(
            str(ctx.exception), u"centennial: Must provide percent alpha acids"
        )
...
class TestHopAdditions(unittest.TestCase):
    ...
    def test_str(self):
        ...
        self.assertEquals(out, u"Centennial, alpha 14.0%, 0.57 oz, 60.0 min, pellet")
    ...
    def test_unicode(self):
        ...
        if sys.version_info[0] >= 3:
            self.assertEquals(
                out, u"Galaxy ®, alpha 11.0%, 0.57 oz, 60.0 min, pellet"
            )  # noqa
        else:
            self.assertEquals(
                out, u"Galaxy ®, alpha 11.0%, 0.57 oz, 60.0 min, pellet".encode("utf8")
            )  # noqa
    ...
    def test_ne_hop_type(self):
        hop_add1 = HopAddition(cascade, boil_time=5.0, weight=0.76)
        hop_add2 = HopAddition(
            cascade, boil_time=5.0, weight=0.76, hop_type=HOP_TYPE_WHOLE
        )
        self.assertTrue(hop_add1 != hop_add2)

    def test_ne_units(self):
        hop_add1 = HopAddition(cascade, boil_time=5.0, weight=0.76)
        hop_add2 = HopAddition(cascade, boil_time=5.0, weight=0.76, units=SI_UNITS)
        self.assertTrue(hop_add1 != hop_add2)
